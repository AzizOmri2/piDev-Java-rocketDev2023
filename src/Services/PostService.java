...
String sql = "SELECT id, title, details,image,date_post,rate,moy_rate FROM post WHERE id = ?";
        try (
             PreparedStatement pstmt = conx.prepareStatement(sql)) {
            pstmt.setInt(1, id);
            ResultSet rs = pstmt.executeQuery();
...
System.out.println(e.getMessage());
    ...
    public void update(Post post, int id) throws SQLException {
        ...
        String sql = "UPDATE post SET title = ?, details = ?,image = ?,rate = ?, date_post = ?, moy_rate = ? WHERE id = ?";
        try ( 
